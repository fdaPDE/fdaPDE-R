...
.geoframe_ctr <- R6::R6Class("geoframe",
  private = list(
  ...
  public = list(
    ...
    print = function(...) {
      ...
      cat("Bounding box:    xmin: ", bbox[1], " ymin: ", bbox[3], " xmax: ", bbox[2], " ymax: ", bbox[4], "\n", sep = "")
      ...
      for (i in seq(from = 1, to = n_layers)) {
        ...
        cat("Dims: ", private$geoframe_$rows(layer_names[i]), ", ", private$geoframe_$cols(layer_names[i]), "\n", sep = "")
        if (private$layer_map_[[i]] == "areal") {
...
.areal_layer <- R6::R6Class("geoframe_areal_layer",
  private = list(
    geoframe_ = "geoframe",
  ...
  public = list(
    ...
    plot = function() {
      ...
      for (i in seq(from = 1, to = length(nodes))) {
        segments(
          x0 = nodes[[i]][edges[[i]][, 1], 1], y0 = nodes[[i]][edges[[i]][, 1], 2],
          x1 = nodes[[i]][edges[[i]][, 2], 1], y1 = nodes[[i]][edges[[i]][, 2], 2]
        )
      }
    },
    print = function() {
    ...
    `[` = function(rows, cols) {
      ...
      gf <- .geoframe_ctr$new()
      gf$.__enclos_env__$private$geoframe_ <- new(cpp_geoframe_2_2, private$geoframe_handler(), private$layer_name_, rows, cols)
    ...
    `[<-` = function(rows, cols, vals) {
      ...
      if (ctype == data_t$flt64) private$geoframe_handler()$flt64_assign(private$layer_name_, rows, cols, as.numeric(vals))
      if (ctype == data_t$flt32) private$geoframe_handler()$flt32_assign(private$layer_name_, rows, cols, as.numeric(vals))
      if (ctype == data_t$int64) private$geoframe_handler()$int64_assign(private$layer_name_, rows, cols, as.integer(vals))
      if (ctype == data_t$int32) private$geoframe_handler()$int32_assign(private$layer_name_, rows, cols, as.integer(vals))
      if (ctype == data_t$bin)   private$geoframe_handler()$bin_assign  (private$layer_name_, rows, cols, as.logical(vals))
      if (ctype == data_t$str)   private$geoframe_handler()$str_assign  (private$layer_name_, rows, cols, as.character(vals))
      invisible(self)
    },
...
.point_layer <- R6::R6Class("geoframe_point_layer",
  private = list(
    geoframe_ = "geoframe",
    layer_name_ = "character",
